...
\collectequation{Caudal e fluxo de massa (aprox. unidimensional)}{
    \dot{m} = \rho Q =\rho V A
}
...
\collectequation{EQ. Transporte Reynolds}{
...
\begin{tcolorbox}[
        ...
        title=\textbf{AVISO: Notação}
    ]
    Na próxima equação, é usada a notação $\vec{p}$ para o momento linear, que não deve ser confundida com a notação usada nos restantes capítulos onde $p$ (normalmente representada sem vetor)representa a pressão.
\end{tcolorbox}
...
\begin{tcolorbox}[
        ...
        title=\textbf{ATENÇÃO: vetor normal e sinais}
    ]
    A convenção para o vetor normal $\hat{n}$ é que este aponta para fora do volume de controlo.
    
    A força de pressão atua no sentido oposto ao vetor normal, daí o sinal negativo no vetor normal. Este pode ser colocado no vetor normal, como em \eqref{eq:pressure-forces-abs} ou fora do integral, como em \eqref{eq:pressure-forces-gage}.
\end{tcolorbox}
...
\collectequation{Força total da pressão (unidimensional)}{
    \vec{F}_{press} = -\left(\left.\sum p_i A_i\right|_{out} - \left.\sum p_i A_i\right|_{in}\right) = \left.\sum p_i A_i\right|_{in} - \left.\sum p_i A_i\right|_{out}
}
